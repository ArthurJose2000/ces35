...
int main(int argc, char *argv[]) {
  ...
  while (!isEnd) {
    ...
    // instancia um objeto da classe HTTPRequest
    HTTPRequest request(input);

    if(request.setURL() == -1){
      perror("Formato incorreto de URL\n");
      return 4;
    }

    string str_request = request.prepareRequest();

    // converte a string lida em vetor de bytes 
    // com o tamanho do vetor de caracteres
    if (send(sockfd, str_request.c_str(), str_request.size(), 0) == -1) {
      perror("send");
      return 4;
    }

    // recebe no buffer uma certa quantidade de bytes ate 1 MB 
    if (recv(sockfd, buf, 1024*1024, 0) == -1) {
      perror("recv");
      return 5;
    }

    request.readResponse(buf);
